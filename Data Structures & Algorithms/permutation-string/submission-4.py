class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        #     - to update matches we first check if after incrementing the counter of the new char in s2 the frequency matches s1
        #     - if it matches we increment
        #     - else we check if it was a match before by checking the incremented freq in s1, if it was a match we decrement the matches as we incresed the freq in sount_s2
        # 6) update the number of matches when increasing the left pointer:
        #     - to update matches we first check if after decrementing the counter of the left pointer in s2 the frequency matches s1
        #     - if it matches we increment
        #     - else we check if it was a match before by checking the decremented freq in s1, if it was a match we decrement the matches as we deincresed the freq in sount_s2        
        
        # 7) return marches == 26 (True if we have found a substring, False otherwise)
